...
def dynamic_batch_size(t: float) -> int:
...
def generate_batch(size: int = 100) -> list[list]:
    ...
    for _ in range(size):
        ...
        quantity = (
            random.randint(1, 5)
            if event_type in ("purchase", "add_to_cart", "refund")
            else 1
        )
...
def main() -> None:
    ...
    print(
        f"Generating events with dynamic batch sizes every {interval}s"
    ...
    try:
        while True:
            ...
            print(
                f"  Inserted {batch_size} events ({purchases} purchases)"
