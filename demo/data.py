...
corresponding repository method in ``chview.db.repository``, using hardcoded
data that mirrors the ``setup.sql`` tables.

Usage::

    from demo.data import demo_schema, demo_materialized_views
"""
...
def demo_materialized_views() -> pd.DataFrame:
    return pd.DataFrame(
        _MVS,
        columns=[
            "database",
...
def demo_storage_metrics() -> pd.DataFrame:
    return pd.DataFrame(
        _STORAGE,
        columns=[
            "database",
...
def demo_partition_storage() -> pd.DataFrame:
    rows = []
...
def demo_mv_throughput(hours: int = 24) -> pd.DataFrame:
    """Generate 5-minute interval throughput data for all 9 MVs."""
    import math
...
def demo_recent_throughput() -> pd.DataFrame:
...
_CREATE_STMTS: dict[str, str] = {}
for _db, _name, _query, _, _ in _MVS:
    _CREATE_STMTS[f"{_db}.{_name}"] = _query
...
def demo_create_table(database: str, table: str) -> str:
    key = f"{database}.{table}"
    return _CREATE_STMTS.get(
        key, f"-- CREATE TABLE statement not available for {key}"
    )
